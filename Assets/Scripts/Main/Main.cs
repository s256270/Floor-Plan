            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 2; j++) {
                    ...
                    foreach (KeyValuePair<string, Vector3[]> room in correctedDwellingPattern[i + j * 5]) {
                        ...
                        //20パターン表示
                        // if (j == 0) {
                        //     GameObject roomObject = cf.CreateRoom(roomName, cf.CorrectCoordinates(correctedDwellingPattern[i + j * 5][roomName], new Vector3(10000f * (i - 2), 3800 * 2.25f, 0)));
                        //     roomObject.transform.SetParent(dwelling.transform);
                        // }
                        // else if (j == 1) {
                        //     GameObject roomObject = cf.CreateRoom(roomName, cf.CorrectCoordinates(correctedDwellingPattern[i + j * 5][roomName], new Vector3(10000f * (i - 2), 3800 * 0.75f, 0)));
                        //     roomObject.transform.SetParent(dwelling.transform);
                        // }
                        // else if (j == 2) {
                        //     GameObject roomObject = cf.CreateRoom(roomName, cf.CorrectCoordinates(correctedDwellingPattern[i + j * 5][roomName], new Vector3(10000f * (i - 2), 3800 * -0.75f, 0)));
